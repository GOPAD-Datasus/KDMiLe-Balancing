...
def smoteenn():
    ...
    smote = SMOTE(sampling_strategy="minority", random_state=72, k_neighbors=7)

    model = SMOTEENN(sampling_strategy="all", random_state=72, enn=enn, smote=smote)
...
def smotetomek():
    ...
    smote = SMOTE(sampling_strategy="minority", random_state=72, k_neighbors=knn)

    model = SMOTETomek(random_state=72, smote=smote, tomek=tomek)
