...
class Runner:
    def __init__(self):
        ...
        df["newborn"] = pca.fit_transform(df[["APGAR5", "RACACOR", "PESO"]])
        df["prenatal"] = pca.fit_transform(
            df[["CONSULTAS", "GESTACAO", "MESPRENAT", "PARTO"]]
        )

        ss = StandardScaler()
        X = pd.DataFrame(ss.fit_transform(df.drop(["OBITO"], axis=1)))
        y = df["OBITO"]

        (self.Xtrain, self.Xtest, self.ytrain, self.ytest) = tts(
            X, y, test_size=0.7, random_state=72
        )
    ...
    def _get_metrics(self, ypred):
        ...
        print(
            classification_report_imbalanced(
                self.ytest, ypred, target_names=target_names
            )
        )
    ...
    def pipeline(self, model):
        Xsample, ysample = model.fit_resample(self.Xtrain, self.ytrain)
